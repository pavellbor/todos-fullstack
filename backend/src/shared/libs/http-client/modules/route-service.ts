...
export class RouteService {
  ...
  public registerRoute(route: Route) {
    this.routesMap[this.getRouteKey(route.method, route.pathname.toString())] = route
  }
  ...
  private async handleRoute(req: Request, res: Response, route: Route) {
    if (route.middlewares) {
      ...
      route.middlewares.forEach((middleware) => middlewareService.registerMiddleware(middleware))
  ...
  private tryToGetRouteWithDynamicParam(req: Request) {
    ...
    for (const routeKey in this.routesMap) {
      ...
      if (routesMapItem.method !== method || routesMapItem.pathname instanceof RegExp) {
        continue
      }
      ...
      if (pathname.slice(0, paramIndex - 1) === routesMapItem.pathname.slice(0, paramIndex - 1)) {
