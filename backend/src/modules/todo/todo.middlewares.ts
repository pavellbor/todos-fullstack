...
export const createParseUserMiddleware = (verifyToken: (token: string) => VerifyRdo) => {
...
export const validateCreateTodoBodyMiddleware = async (req: Request, res: Response) => {
  const { title } = req.body as CreateTodoDto

  if (!title) {
    throw new HttpError(StatusCodes.BAD_REQUEST, 'Текст задачи не может быть пустым')
  }
}
