...
import { DatabaseClient, DatabaseData } from '../../shared/libs/database-client'
...
export const createTodoController = (userService: UserService, loggerService: LoggerService) => {
  const configService = new ConfigService<{ DB_TODOS_PATH: string }>()
  const fileManager = new FileManager<DatabaseData<Todo>>(configService.get('DB_TODOS_PATH'))
  const databaseClient = new DatabaseClient<Todo>(fileManager)
  const todoService = new TodoService(databaseClient)
  const todoController = new TodoController(userService, todoService, loggerService)
